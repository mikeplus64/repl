...
import Data.IORef
import Data.Maybe
import Data.List (dropWhileEnd, takeWhile, union)
...
repl :: [String]        -- ^ Imports, using normal Haskell import syntax.
     ...
     -> IO Repl
repl imports extensions input output = do
    let send :: MonadIO m => [String] -> m ()
        send = liftIO . writeChan output
        line = liftIO (readChan input)
    interp <- forkIO $
        runGhc (Just libdir) $ do
            dflags <- session extensions
            forever $ do
                import_ imports
                x   <- line
                msg <- liftIO (newIORef [])
                ...
                forM_ (init x) (\s -> 
                    (if isDecl s 
                        then void $ runDecls s
                        else void $ runStmt ("let {" ++ s ++ "}") SingleStep)
                    `gcatch` 
                ...
                send =<< liftIO (readIORef msg)

    return $ Repl input output interp
  where
    ...
    session exts = do
        ...
        setSessionDynFlags (getExts exts s)
        getSessionDynFlags
